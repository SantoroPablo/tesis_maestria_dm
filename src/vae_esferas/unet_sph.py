def correr_unet(data_name, num_run, z_dim, use_kldiv=True, sign_kldiv=1,
                mult_kldiv=1, epochs=5):
    """
    VAE para las esferas 3D, algo parecido a las UNet pero sin hacer las
    conexiones directas entre capa de encoding y de decoding
    Parametros:
    ...
    model = MyUNet(z_dim=z_dim).to(device)
    ...
    torch.save(model, RUTA_REPO + 'modelos/sph_unet_run_' + str(num_run) +
               '.pth')
